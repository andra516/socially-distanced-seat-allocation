...
def jonsAllocator(points, scale):
    ''' 
    Jon's algorithm for seat allocation
...
#                    seeds[c] = np.random.choice(I, 1)
        ...
        for s in np.arange(n2):
            count2 = maxFoundSoFar//5 + np.random.randint(1, maxFoundSoFar//4+1)
...
#                    # Only if this aSeeds seat if futher than 2m away from original seat - not a problem if margin=0
        ...
        print(f'max found after stage2: {numberAllocated[reRun]}')
...
def main():
    global seatsAllocatedBest, positionsBest, allocatedPositions, seatsAllocatedBestBest, positionsBestBest
    ...
    maxFound = 0
    for _ in range(100):
#    jonsAllocator(seatPixelCoords, magicScale)
        seatsAllocatedBest, positionsBest = richardsAllocator(seatPixelCoords, stageOneLoops=n1, stageTwoLoops=n2, margin=margin, totalReRuns=1)
        allocatedPositions = np.take(positionsBest, seatsAllocatedBest, axis=0)
        
        
        overlapCount += checkOverlap(allocatedPositions)
        if len(seatsAllocatedBest) > maxFound:
            maxFound = len(seatsAllocatedBest)
            seatsAllocatedBestBest = seatsAllocatedBest
            positionsBestBest = positionsBest
